...
func (esshash *Ethash) Seal(chain consensus.ChainReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	...
	if esshash.config.PowMode == ModeFake || esshash.config.PowMode == ModeFullFake {
		...
		default:
			log.Warn("Sealing result is not read by miner", "mode", "fake", "sealhash", esshash.SealHash(block.Header()))
		}
		return nil
	}
	// If we're running a shared PoW, delegate sealing to it
	if esshash.shared != nil {
		return esshash.shared.Seal(chain, block, results, stop)
	}
	...
	if esshash.workCh != nil {
		esshash.workCh <- &sealTask{block: block, results: results}
	}
	var (
		pend   sync.WaitGroup
	...
	for i := 0; i < threads; i++ {
		...
		go func(id int, nonce uint64) {
			defer pend.Done()
			esshash.mine(block, id, nonce, abort, locals)
	...
	go func() {
		...
		case result = <-locals:
			...
			case results <- result:
			default:
				log.Warn("Sealing result is not read by miner", "mode", "local", "sealhash", esshash.SealHash(block.Header()))
			}
			close(abort)
		case <-esshash.update:
			// Thread count was changed on user request, restart
			close(abort)
			if err := esshash.Seal(chain, block, results, stop); err != nil {
...
func (esshash *Ethash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
	...
	var (
		header  = block.Header()
		hash    = esshash.SealHash(header).Bytes()
...
func (esshash *Ethash) remote(notify []string, noverify bool) {
	var (
	...
	makeWork := func(block *types.Block) {
		hash := esshash.SealHash(block.Header())
