...
func TestHeaderVerification(t *testing.T) {
	...
	chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, esshash.NewFaker(), vm.Config{}, nil)
	defer chain.Stop()
...
func testHeaderConcurrentVerification(t *testing.T, threads int) {
	...
	for i, valid := range []bool{true, false} {
		...
		if valid {
			...
			chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, esshash.NewFakeFailer(uint64(len(headers)-1)), vm.Config{}, nil)
			_, results = chain.engine.VerifyHeaders(chain, headers, seals)
			chain.Stop()
		}
...
func testHeaderConcurrentAbortion(t *testing.T, threads int) {
	...
	chain, _ := NewBlockChain(testdb, nil, params.TestChainConfig, esshash.NewFakeDelayer(time.Millisecond), vm.Config{}, nil)
	defer chain.Stop()
