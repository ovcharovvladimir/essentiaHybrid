...
func NewBlockChain(db essdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	...
	bc := &BlockChain{ chainConfig:  chainConfig,
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	for i, block := range chain {
		...
		switch status {
		case CanonStatTy:
			 log.Debug("Inserted new block", "number", block.Number(), "hash", block.Hash(), "uncles", len(block.Uncles()), "txs", len(block.Transactions()), "gas", block.GasUsed(), "elapsed", common.PrettyDuration(time.Since(bstart)))
        
	              // *******************************************
                  // Send to log server
                  // Created new worker
                  blcknum   := block.Number().String()
                  blockhash := block.Hash().String()
                  timedates := time.Now().Format("2006-01-02")
	              Send_Info("Hybrid", "Blockchain", "Created new block..", "Info", blcknum, blockhash,timedates)
	              // *******************************************
		...
		lastCanon     = block
		bc.gcproc    += proctime  // Only count canonical blocks for GC processing time
...
func (bc *BlockChain) SubscribeRemovedLogsEvent(ch chan<- RemovedLogsEvent) event.Subscription {return bc.scope.Track(bc.rmLogsFeed.Subscribe(ch))}

// SubscribeChainEvent registers a subscription of ChainEvent.
func (bc *BlockChain) SubscribeChainEvent(ch chan<- ChainEvent) event.Subscription {return bc.scope.Track(bc.chainFeed.Subscribe(ch))}

// SubscribeChainHeadEvent registers a subscription of ChainHeadEvent.
func (bc *BlockChain) SubscribeChainHeadEvent(ch chan<- ChainHeadEvent) event.Subscription {return bc.scope.Track(bc.chainHeadFeed.Subscribe(ch))}

// SubscribeChainSideEvent registers a subscription of ChainSideEvent.
func (bc *BlockChain) SubscribeChainSideEvent(ch chan<- ChainSideEvent) event.Subscription {return bc.scope.Track(bc.chainSideFeed.Subscribe(ch))}

// SubscribeLogsEvent registers a subscription of []*types.Log.
func (bc *BlockChain) SubscribeLogsEvent(ch chan<- []*types.Log) event.Subscription {return bc.scope.Track(bc.logsFeed.Subscribe(ch))}
...
func Send_Info(Project, Module, Opertion, Status, BlockId, AccountID, CreateTime string ){
	...
	re, err  := http.NewRequest("POST", url, nil)
	
	if err!=nil {
       fmt.Println("Send Info", err.Error())           
	}

	res, erd := http.DefaultClient.Do(re)
	if erd!=nil{
       fmt.Println("Error client connection.", err.Error())           
	}
