...
RUN \
  echo 'gess --cache 512 init /genesis.json' > gess.sh && \{{if .Unlock}}
	...
	echo $'exec gess --networkid {{.NetworkID}} --cache 512 --port {{.Port}} --maxpeers {{.Peers}} {{.LightFlag}} --ethstats \'{{.Ethstats}}\' {{if .Bootnodes}}--bootnodes {{.Bootnodes}}{{end}} {{if .Etherbase}}--miner.etherbase {{.Etherbase}} --mine --miner.threads 1{{end}} {{if .Unlock}}--unlock 0 --password /signer.pass --mine{{end}} --miner.gastarget {{.GasTarget}} --miner.gasprice {{.GasPrice}}' >> gess.sh
...
func (info *nodeInfos) Report() map[string]string {
	...
	if info.gasTarget > 0 {
		...
		if info.etherbase != "" {
			...
			report["Esshash directory"] = info.ethashdir
...
func checkNode(client *sshClient, network string, boot bool) (*nodeInfos, error) {
	...
	stats := &nodeInfos{
		...
		ethashdir:  infos.volumes["/root/.esshash"],
		port:       port,
	...
	stats.enode = fmt.Sprintf("essnode://%s@%s:%d", id, client.address, stats.port)
