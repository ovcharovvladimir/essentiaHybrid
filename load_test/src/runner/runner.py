...
def run(nodes_count, load_factor):
    ...
    maximal_nodes_count = len(GessNodes())
...
def custom_run(nodes, load_factor):
    minimal_nodes_count = 1
    maximal_nodes_count = len(GessNodes())

    pool = Pool(len(nodes))

    nodes_count = clamp(len(nodes), minimal_nodes_count, maximal_nodes_count)
    gess_nodes_for_the_run = nodes
...
def test_run(node_index, load_factor):
    _spawn_single_run(node_index=1, load_factor=load_factor, gess_nodes_for_the_run=CycleList(GessNodes()[:2]))
