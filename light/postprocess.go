...
type ChtIndexerBackend struct {
	diskdb, trieTable    essdb.Database
	odr                  OdrBackend
...
func NewChtIndexer(db essdb.Database, odr OdrBackend, size, confirms uint64) *core.ChainIndexer {
	trieTable := essdb.NewTable(db, ChtTablePrefix)
	...
	return core.NewChainIndexer(db, essdb.NewTable(db, "chtIndex-"), backend, size, confirms, time.Millisecond*100, "cht")
}
...
func NewBloomTrieIndexer(db essdb.Database, odr OdrBackend, parentSize, size uint64) *core.ChainIndexer {
	trieTable := essdb.NewTable(db, BloomTrieTablePrefix)
	...
	return core.NewChainIndexer(db, essdb.NewTable(db, "bltIndex-"), backend, size, 0, time.Millisecond*100, "bloomtrie")
}
